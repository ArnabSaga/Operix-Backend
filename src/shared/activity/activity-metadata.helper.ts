...
const activityMetadataAllowlist: Record<string, ReadonlySet<string>> = {
  ...
  TASK_DISTRIBUTION_SCHEDULED: new Set([]),
  TASK_DISTRIBUTION_RESCHEDULED: new Set([]),
  TASK_DISTRIBUTION_CANCELLED: new Set([]),
  TASK_DISTRIBUTION_SENT: new Set(['taskId', 'scope', 'dueAt']),
  TASK_SELF_CLAIM_ENABLED: new Set([]),
  TASK_SELF_CLAIM_DISABLED: new Set([]),
  TASK_CLAIMED: new Set([]),
